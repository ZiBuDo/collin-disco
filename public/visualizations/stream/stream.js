...
var mShadowColor = 0x000000; //0x1B0914
...
var mAudioElement = document.getElementById('audio');
var mAnalyser;

setTimeout(() => {
    mAudioElement.currentTime = 0;
    initAudio();
    mAudioElement.play().then(() => {
        init();
...
function initAudio() {
    ...
    mAudioElement.src = 'http://localhost:3000/tune.mp3';
    mAnalyser = new SpectrumAnalyzer(mPathLength * 0.5, 0.80);
    mAnalyser.setSource(mAudioElement);
}

function initTHREE() {
    mRenderer = new THREE.WebGLRenderer({ antialias: mUseAA, alpha: true });
...
function initParticleSystem() {
    ...
    var minDuration = mAudioElement.duration * 1.1;
    var maxDuration = mAudioElement.duration * 2;
